...
void main() {
    ...
    vec4 leftTile = texture(texture0, vec2(tileCoord.x - 1.0, tileCoord.y) * texelSize);
    vec4 rightTile = texture(texture0, vec2(tileCoord.x + 1.0, tileCoord.y) * texelSize);
    vec4 topTile = texture(texture0, vec2(tileCoord.x, tileCoord.y + 1.0) * texelSize);
    vec4 bottomTile = texture(texture0, vec2(tileCoord.x, tileCoord.y - 1.0) * texelSize);
    ...
    finalTileState.w = finalVel.y * 0.5 + 0.5;
}